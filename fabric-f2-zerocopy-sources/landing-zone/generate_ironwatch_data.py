"""Landing-zone synthetic data generator for the OneLake Shortcut demo.

Adapted from ironwatch-v1/synthetic_data/generators/generate_all.py: same
SEED, schema, and generation logic for all 5 file types, but scaled down
(equipment_count ~10, days ~30) so the output is landing-zone-appropriate
in size before being uploaded to the "ironwatch-landing" ADLS Gen2
container.
"""
...
def load_config():
    load_dotenv()
    default_output = str(Path(__file__).resolve().parent / "output")
    output_path = os.getenv("LANDING_OUTPUT_PATH", default_output)
    return {
        "equipment_count": int(os.getenv("LANDING_EQUIPMENT_COUNT", "10")),
        "days": int(os.getenv("LANDING_DAYS", "30")),
...
def main():
    ...
    print("IronWatch landing-zone sample - synthetic data generation")
